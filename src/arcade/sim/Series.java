...
public class Series {
	...
	private static final String[] COORD_NAMES = new String[] { "Hexagonal" };
	...
	/** Specification for cell initialization structure */
	String _plate;
	 ...
	 * 
	 * @return  the type of coordiante system as String.
	 */
	public String getCoordString() { return COORD_NAMES[_coord]; }
	...
	private void updateAgents(MiniBox agents) {
		...
		String plate;
		if (agents.contains("plate")) { plate = agents.get("plate"); }
		else { plate = "unspecified"; }
		
		// Set plating of agents to 0 for spherical, 1 for dish;
		if (plate.equals("spheroid")) { _plate = "spheroid"; }
		else if (plate.equals("dish")) { _plate = "dish"; }
		else if (plate.equals("unspecified")) {
			plate = "spheroid";
			LOGGER.warning("plating not specified, default spheroid for [ " + name + " ] ");
		}
		else {
			plate = "spheroid";
			LOGGER.warning("plating specification invalid, default spheroid for [ " + name + " ] ");
		}
		
		if (plate.equals("spheroid")) {
		...
		else if (plate.equals("dish")) {
			// Check if init is an integer greater than or equal to 0. If init is given 
			// as "FULL", then each location will get one cell.
			if (init.matches("[0-9]+") && Integer.parseInt(init) >= 0) { _init = Integer.parseInt(init); }
			else if (init.toUpperCase().equals("FULL")) { _init = FULL_INIT; }
			else {
				LOGGER.warning("initialization [ " + init
					+ " ] must be FULL or greater than or equal to 0");
				skip = true;
			}
		}
	}
	...
	private void updatePopulations(ArrayList<MiniBox> populations, ArrayList<ArrayList<MiniBox>> modules) {
		...
		for (int i = 0; i < _pops; i++) {
			...
			if (box.get("metabolism") == null && !type.equals("4") && !type.equals("8")) {
			...
			if (box.get("signaling") == null && !type.equals("4") && !type.equals("8")) {
			...
			if (type.equals("4")) {
				box.put("metabolism", "CAR T-CELL");
				box.put("signaling", "null");
				box.put("inflammation", "CD4");
			}
			
			if (type.equals("8")) {
				box.put("metabolism", "CAR T-CELL");
				box.put("signaling", "null");
				box.put("inflammation", "CD8");
			}
			...
			try {
				if (type.equals("H") || type.equals("C") || type.equals("S")) {
					...
					Class<?> c = Class.forName("arcade.agent.cell.CART" + type + "Cell");
		...
		for (MiniBox h : helpers) {
			...
			switch (h.get("type").toLowerCase()) {
				...
				case "wound":
					...
					break;
				case "treat":
					// Determine how many CAR populations were created
					// that were initialized at a ratio of 0.
					int CARs = 0;
					for (int c = 0; c < _pops; c ++) {
						try {
							if ( ( _popCons[c].equals(Class.forName("arcade.agent.cell.CART4Cell").getConstructors()[0]) 
									|| _popCons[c].equals(Class.forName("arcade.agent.cell.CART8Cell").getConstructors()[0]) ) && _popFrac[c] == 0) {
									CARs++;
								}
						} catch (ClassNotFoundException e) { e.printStackTrace(); }
					}
					
					// If no CAR populations were specified in initialization at ratio 0,
					// cannot complete treatment.
					if ( CARs == 0 ) { LOGGER.warning("No CAR T-cell populations were created in initialization -- cannot complete treatment"); }
					
					// Otherwise proceed.
					else {
						int[] treatPops = new int[CARs];
						Constructor<?>[] treatCons = new Constructor<?>[CARs];
						double[] treatFrac = new double[CARs];
						
						// Check for specified ratio of CARs upon treatment.
						if (h.contains("ratio")) { 
							String[] tr = h.get("ratio").split(":");
							
							// Check to see if sum of ratios equals 1.
							double sum = 0;
							for (int n = 0; n < tr.length; n++) { sum += Double.parseDouble(tr[n]); }
							
							// If not specified or if the wrong number of ratios or 
							// total ratios for specified number of populations was specified,
							// then use an even ratio of each specified population.
							if ( tr.length != CARs || sum != 1.0) {
								LOGGER.warning("Improper number of ratios specified, will use even ratios of each");
								double ratio = 1.0 / CARs;
								for (int j = 0; j < CARs; j++ ) { treatFrac[j] = ratio; }
							}
							
							// If proper ratio specified,
							// assign ratios to corresponding populations.
							else {
								for (int j = 0; j < CARs; j++ ) { treatFrac[j] = Double.parseDouble(tr[j]); }
							}
						}

						// If no ratio was specified at all, 
						// use an even ratio of each specified population.
						else {
							LOGGER.warning("No ratios specified, will use even ratios of each");
							double ratio = 1.0 / CARs;
							for (int j = 0; j < CARs; j++ ) { treatFrac[j] = ratio; }
						}
						
						// Copy over which cell types are desired upon treatment and their
						// corresponding population numbers and constructors. 
						int n = 0;
						for (int m = 0; m < _pops; m ++) {
							try {
								if ( (_popCons[m].equals(Class.forName("arcade.agent.cell.CART4Cell").getConstructors()[0]) 
									|| _popCons[m].equals(Class.forName("arcade.agent.cell.CART8Cell").getConstructors()[0]) ) && _popFrac[m] == 0) {
										treatPops[n] = m;
										treatCons[n] = _popCons[m];
										n++;
								}
							} catch (ClassNotFoundException e) { e.printStackTrace(); }
						}
						
						// Check specified dose
						int dose;
						if (h.contains("dose")) { 
							dose = h.getInt("dose");
						}
						else { dose = 500; }
						_helpers.add(new TreatHelper(h.getInt("delay"), dose, treatPops, treatCons, treatFrac, getCoordString()));
						LOGGER.info("Adding TREAT helper to <" + name + "> with CD4:CD8 ratio of " + h.get("ratio"));
					}
					break;
				default:
		...
		for (MiniBox p : profilers) {
			...
			switch (p.get("type").toLowerCase()) {
				...
				case "graph":
					...
					break;
				case "lysis":
					_profilers.add(new LysisProfiler(i, suffix));
					LOGGER.info(String.format(profilerFormat, "LYSIS", name));
					break;
				default:
	...
	public String configToJSON() {
		...
		for (int i = 0; i < _pops; i++) {
			if (_popCons[i].getName().equals("arcade.agent.cell.CART4Cell") || 
					_popCons[i].getName().equals("arcade.agent.cell.CART8Cell")) {
				pops += String.format(cf, i, _popCons[i].getName(), _popBoxes[i].get("metabolism").charAt(4),
						_popBoxes[i].get("inflammation").charAt(2), _popFrac[i], _popCounts[i]);
			}
			else {
	...
	public String toString() {
		...
		for (int i = 0; i < _pops; i++) {
			...
			if (_popCons[i].getName().equals("arcade.agent.cell.CART4Cell") || 
					_popCons[i].getName().equals("arcade.agent.cell.CART8Cell")) {
				cells += String.format("\t%10s : %s (meta = %s, inf = %s) (%.2f", title,
						_popCons[i].getName(), box.get("metabolism"), box.get("inflammation"),
						100*_popFrac[i]) + "%)\n";
			}
			else {
