...
public abstract class TissueCell implements Cell {
	...
	protected int CAR_ANTIGENS;
	
	/** Cell surface PDL1 count */
	private int SELF_TARGETS;
	...
	public TissueCell(int pop, Location loc, double vol, int age, Map<String, Parameter> p) {
		...
		this.CAR_ANTIGENS = p.get("CAR_ANTIGENS_HEALTHY").nextInt();
		this.SELF_TARGETS = p.get("SELF_TARGETS").nextInt();
		...
		params.put("CAR_ANTIGENS_HEALTHY", p.get("CAR_ANTIGENS_HEALTHY").update(CAR_ANTIGENS));
		params.put("CAR_ANTIGENS_CANCER", p.get("CAR_ANTIGENS_CANCER"));
		params.put("SELF_TARGETS", p.get("SELF_TARGETS").update(SELF_TARGETS));
	}
	...
	public int getCARAntigens() { return CAR_ANTIGENS; }
	public int getSelfTargets() { return SELF_TARGETS; }
