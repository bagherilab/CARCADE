...
public abstract class GrowthVisualization2D extends Visualization {
	...
	public static class Hexagonal extends GrowthVisualization2D {
		...
		public Drawer[] createDrawers() {
			return new Drawer[] {
					...
					new EnvDrawer2D.Triangular(panels[1], "IL-2",
							LENGTH, WIDTH, DEPTH, maps.MAP_IL2, getBox(0,600,200)),
					...
					new AuxDrawer2D.Label(panels[1], "label", 0, 50, "[TGFÎ±]", false),
					new AuxDrawer2D.Label(panels[1], "label", 0, 75, "[IL-2]", false),
					...
					new AgentDrawer2D.Hexagonal(panels[2], "count",
							LENGTH, WIDTH, DEPTH, maps.MAP_COUNT, getBox(200,0,200), GRID_DOUBLE),
	...
	public Panel[] createPanels() {
		return new Panel[]{
				...
				new Panel.Panel2D("[2D] Environment", 440, 10, 600, 800, this),
